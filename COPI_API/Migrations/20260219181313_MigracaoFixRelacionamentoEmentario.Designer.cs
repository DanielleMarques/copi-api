// <auto-generated />
using System;
using COPI_API;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace COPI_API.Migrations
{
    [DbContext(typeof(AppDbContext))]
    [Migration("20260219181313_MigracaoFixRelacionamentoEmentario")]
    partial class MigracaoFixRelacionamentoEmentario
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "8.0.13")
                .HasAnnotation("Relational:MaxIdentifierLength", 64);

            modelBuilder.Entity("COPI_API.Models.AdminEntities.Divisao", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<string>("Nome")
                        .HasColumnType("longtext");

                    b.HasKey("Id");

                    b.ToTable("Divisoes");
                });

            modelBuilder.Entity("COPI_API.Models.AdminEntities.Servidor", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<string>("CargoOuFuncao")
                        .HasColumnType("longtext");

                    b.Property<string>("Celular")
                        .HasColumnType("longtext");

                    b.Property<string>("ChefiaImediata")
                        .HasColumnType("longtext");

                    b.Property<DateTime>("DataCriacao")
                        .HasColumnType("datetime(6)");

                    b.Property<int>("DivisaoId")
                        .HasColumnType("int");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<string>("HorarioEntrada")
                        .HasColumnType("longtext");

                    b.Property<string>("HorarioSaida")
                        .HasColumnType("longtext");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<string>("PasswordHash")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<string>("PontoSei")
                        .HasColumnType("longtext");

                    b.Property<string>("RF")
                        .HasColumnType("longtext");

                    b.Property<string>("Role")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<string>("Status")
                        .HasColumnType("longtext");

                    b.Property<DateTime?>("UltimoAcesso")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("UserName")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.HasKey("Id");

                    b.HasIndex("DivisaoId");

                    b.ToTable("Servidores");
                });

            modelBuilder.Entity("COPI_API.Models.AdminEntities.Unidade", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<string>("Nome")
                        .HasColumnType("longtext");

                    b.Property<string>("NomeAbreviado")
                        .HasColumnType("longtext");

                    b.Property<string>("Tipo")
                        .HasColumnType("longtext");

                    b.HasKey("Id");

                    b.ToTable("Unidades");
                });

            modelBuilder.Entity("COPI_API.Models.DPEEntities.Afastamento", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<DateTimeOffset>("CriadoEm")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("CriadoPor")
                        .HasColumnType("longtext");

                    b.Property<DateTime>("DataFim")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime>("DataInicio")
                        .HasColumnType("datetime(6)");

                    b.Property<int>("DeclaracaoPdfId")
                        .HasColumnType("int");

                    b.Property<int>("ManifestacaoPdfId")
                        .HasColumnType("int");

                    b.Property<string>("NumeroProcesso")
                        .HasColumnType("longtext");

                    b.Property<bool>("PossuiInconsistencia")
                        .HasColumnType("tinyint(1)");

                    b.Property<string>("ServidorCargo")
                        .HasColumnType("longtext");

                    b.Property<string>("ServidorMatricula")
                        .HasColumnType("longtext");

                    b.Property<string>("ServidorNome")
                        .HasColumnType("longtext");

                    b.Property<int>("Status")
                        .HasColumnType("int");

                    b.Property<int>("Tipo")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.ToTable("Afastamentos");
                });

            modelBuilder.Entity("COPI_API.Models.DPEEntities.Documento", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("char(36)");

                    b.Property<int>("AfastamentoId")
                        .HasColumnType("int");

                    b.Property<bool>("Anonimizado")
                        .HasColumnType("tinyint(1)");

                    b.Property<string>("CaminhoArquivo")
                        .HasColumnType("longtext");

                    b.Property<string>("ContentType")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<DateTimeOffset>("CriadoEm")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("CriadoPor")
                        .HasColumnType("longtext");

                    b.Property<int>("DivisaoId")
                        .HasColumnType("int");

                    b.Property<string>("NomeArmazenado")
                        .HasColumnType("longtext");

                    b.Property<string>("NomeOriginal")
                        .HasColumnType("longtext");

                    b.Property<long>("Tamanho")
                        .HasColumnType("bigint");

                    b.Property<string>("TextoExtraido")
                        .HasColumnType("longtext");

                    b.Property<int>("Tipo")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("AfastamentoId");

                    b.HasIndex("DivisaoId");

                    b.ToTable("Documentos");
                });

            modelBuilder.Entity("COPI_API.Models.DPEEntities.Ementario", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<int>("AfastamentoId")
                        .HasColumnType("int");

                    b.Property<DateTimeOffset>("CriadoEm")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("EmentaResumo")
                        .HasColumnType("longtext");

                    b.Property<string>("Fundamentacao")
                        .HasColumnType("longtext");

                    b.Property<string>("NumeroSei")
                        .HasColumnType("longtext");

                    b.HasKey("Id");

                    b.HasIndex("AfastamentoId")
                        .IsUnique();

                    b.ToTable("Ementarios");
                });

            modelBuilder.Entity("COPI_API.Models.MetaEntities.AcaoEstrategica", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<string>("AcaoExecutada")
                        .HasColumnType("longtext");

                    b.Property<bool>("Batida")
                        .HasColumnType("tinyint(1)");

                    b.Property<string>("Comentarios")
                        .HasColumnType("longtext");

                    b.Property<bool>("Concluido")
                        .HasColumnType("tinyint(1)");

                    b.Property<DateTime?>("DataCumprimento")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("Descricao")
                        .HasColumnType("longtext");

                    b.Property<bool>("DocumentoValidado")
                        .HasColumnType("tinyint(1)");

                    b.Property<string>("DocumentosAnalise")
                        .HasColumnType("longtext");

                    b.Property<string>("Evidencia")
                        .HasColumnType("longtext");

                    b.Property<int>("MetaId")
                        .HasColumnType("int");

                    b.Property<DateTime?>("PrazoFinal")
                        .HasColumnType("datetime(6)");

                    b.Property<int>("Progresso")
                        .HasColumnType("int");

                    b.Property<string>("ResponsavelAprovacao")
                        .HasColumnType("longtext");

                    b.Property<string>("ResponsavelExecucao")
                        .HasColumnType("longtext");

                    b.Property<string>("Status")
                        .HasColumnType("longtext");

                    b.Property<string>("Titulo")
                        .HasColumnType("longtext");

                    b.HasKey("Id");

                    b.HasIndex("MetaId");

                    b.ToTable("AcoesEstrategicas");
                });

            modelBuilder.Entity("COPI_API.Models.MetaEntities.Meta", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<string>("Aprovador")
                        .HasColumnType("longtext");

                    b.Property<string>("AvaliacaoDi")
                        .HasColumnType("longtext");

                    b.Property<string>("Consultado")
                        .HasColumnType("longtext");

                    b.Property<DateTime?>("DataCumprimento")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime>("DataFim")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime>("DataInic")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("Descricao")
                        .HasColumnType("longtext");

                    b.Property<int>("DivisaoId")
                        .HasColumnType("int");

                    b.Property<string>("Entregavel")
                        .HasColumnType("longtext");

                    b.Property<string>("Informado")
                        .HasColumnType("longtext");

                    b.Property<string>("Objetivo")
                        .HasColumnType("longtext");

                    b.Property<string>("OrigemMeta")
                        .HasColumnType("longtext");

                    b.Property<string>("Premissas")
                        .HasColumnType("longtext");

                    b.Property<int>("Progresso")
                        .HasColumnType("int");

                    b.Property<string>("Responsavel")
                        .HasColumnType("longtext");

                    b.Property<string>("Restricoes")
                        .HasColumnType("longtext");

                    b.Property<string>("Riscos")
                        .HasColumnType("longtext");

                    b.Property<string>("Setor")
                        .HasColumnType("longtext");

                    b.Property<string>("Status")
                        .HasColumnType("longtext");

                    b.Property<string>("Tipo")
                        .HasColumnType("longtext");

                    b.Property<string>("Titulo")
                        .HasColumnType("longtext");

                    b.HasKey("Id");

                    b.ToTable("Metas");
                });

            modelBuilder.Entity("COPI_API.Models.MetaEntities.Tarefa", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<int>("AcaoEstrategicaId")
                        .HasColumnType("int");

                    b.Property<string>("AvaliacaoDoc")
                        .HasColumnType("longtext");

                    b.Property<bool>("Batida")
                        .HasColumnType("tinyint(1)");

                    b.Property<string>("Comentario")
                        .HasColumnType("longtext");

                    b.Property<DateTime?>("DataCumprimento")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("Descricao")
                        .HasColumnType("longtext");

                    b.Property<DateTime?>("PrazoFinal")
                        .HasColumnType("datetime(6)");

                    b.Property<int>("Progresso")
                        .HasColumnType("int");

                    b.Property<string>("Responsavel")
                        .HasColumnType("longtext");

                    b.Property<string>("Status")
                        .HasColumnType("longtext");

                    b.Property<string>("StatusExecucao")
                        .HasColumnType("longtext");

                    b.Property<string>("Titulo")
                        .HasColumnType("longtext");

                    b.HasKey("Id");

                    b.HasIndex("AcaoEstrategicaId");

                    b.ToTable("Tarefas");
                });

            modelBuilder.Entity("COPI_API.Models.PIBPEntities.Avaliacao", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<DateTime>("DataCriacao")
                        .HasColumnType("datetime(6)");

                    b.Property<bool>("NivelGerenciadoAtingido")
                        .HasColumnType("tinyint(1)");

                    b.Property<bool>("NivelIntegradoAtingido")
                        .HasColumnType("tinyint(1)");

                    b.Property<bool>("NivelPadronizadoAtingido")
                        .HasColumnType("tinyint(1)");

                    b.Property<decimal>("NotaIMPIBP")
                        .HasColumnType("decimal(65,30)");

                    b.Property<decimal>("NotaNG")
                        .HasColumnType("decimal(65,30)");

                    b.Property<decimal>("NotaNI")
                        .HasColumnType("decimal(65,30)");

                    b.Property<decimal>("NotaNP")
                        .HasColumnType("decimal(65,30)");

                    b.HasKey("Id");

                    b.ToTable("Avaliacoes");
                });

            modelBuilder.Entity("COPI_API.Models.PIBPEntities.Ciclo", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<DateTime?>("DataFim")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime?>("DataInicio")
                        .HasColumnType("datetime(6)");

                    b.Property<bool?>("Encerrado")
                        .HasColumnType("tinyint(1)");

                    b.Property<string>("Nome")
                        .HasColumnType("longtext");

                    b.HasKey("Id");

                    b.ToTable("Ciclos");
                });

            modelBuilder.Entity("COPI_API.Models.PIBPEntities.Eixo", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<string>("Nome")
                        .HasColumnType("longtext");

                    b.Property<decimal>("Peso")
                        .HasColumnType("decimal(65,30)");

                    b.HasKey("Id");

                    b.ToTable("Eixos");
                });

            modelBuilder.Entity("COPI_API.Models.PIBPEntities.KPI", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<int?>("EixoId")
                        .HasColumnType("int");

                    b.Property<int?>("NivelId")
                        .HasColumnType("int");

                    b.Property<string>("Nome")
                        .HasColumnType("longtext");

                    b.Property<double?>("Pontuacao")
                        .HasColumnType("double");

                    b.HasKey("Id");

                    b.HasIndex("EixoId");

                    b.HasIndex("NivelId");

                    b.ToTable("KPIs");
                });

            modelBuilder.Entity("COPI_API.Models.PIBPEntities.Nivel", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<string>("Nome")
                        .HasColumnType("longtext");

                    b.Property<decimal>("Valor")
                        .HasColumnType("decimal(65,30)");

                    b.HasKey("Id");

                    b.ToTable("Niveis");
                });

            modelBuilder.Entity("COPI_API.Models.PIBPEntities.ResultadoKPI", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<string>("AvaliacaoEscrita")
                        .HasColumnType("longtext");

                    b.Property<int?>("AvaliacaoId")
                        .HasColumnType("int");

                    b.Property<int>("CicloId")
                        .HasColumnType("int");

                    b.Property<DateTime>("DataRegistro")
                        .HasColumnType("datetime(6)");

                    b.Property<int>("KPIId")
                        .HasColumnType("int");

                    b.Property<string>("Prova")
                        .HasColumnType("longtext");

                    b.Property<string>("Status")
                        .HasColumnType("longtext");

                    b.Property<DateTimeOffset?>("UltimaAlteracaoEm")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("UltimaAlteracaoPor")
                        .HasColumnType("longtext");

                    b.Property<int>("UnidadeKPIId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("AvaliacaoId");

                    b.HasIndex("CicloId");

                    b.HasIndex("KPIId");

                    b.HasIndex("UnidadeKPIId");

                    b.ToTable("ResultadosKPI");
                });

            modelBuilder.Entity("COPI_API.Models.PIBPEntities.StatusPIBP", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<int>("CicloId")
                        .HasColumnType("int");

                    b.Property<int>("Status")
                        .HasColumnType("int");

                    b.Property<int>("UnidadeKPIId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("CicloId");

                    b.HasIndex("UnidadeKPIId");

                    b.ToTable("StatusPIBP");
                });

            modelBuilder.Entity("COPI_API.Models.PIBPEntities.UnidadeKPI", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<string>("SEI")
                        .HasColumnType("longtext");

                    b.Property<int>("ServidorId")
                        .HasColumnType("int");

                    b.Property<int>("UnidadeId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("ServidorId");

                    b.HasIndex("UnidadeId");

                    b.ToTable("UnidadesKPI");
                });

            modelBuilder.Entity("COPI_API.Models.PIGEEntities.AvaliacaoPIGE", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<DateTime>("DataCriacao")
                        .HasColumnType("datetime(6)");

                    b.Property<bool>("NivelGerenciadoAtingido")
                        .HasColumnType("tinyint(1)");

                    b.Property<bool>("NivelIntegradoAtingido")
                        .HasColumnType("tinyint(1)");

                    b.Property<bool>("NivelPadronizadoAtingido")
                        .HasColumnType("tinyint(1)");

                    b.Property<decimal>("NotaIMPIGE")
                        .HasColumnType("decimal(65,30)");

                    b.Property<decimal>("NotaNG")
                        .HasColumnType("decimal(65,30)");

                    b.Property<decimal>("NotaNI")
                        .HasColumnType("decimal(65,30)");

                    b.Property<decimal>("NotaNP")
                        .HasColumnType("decimal(65,30)");

                    b.HasKey("Id");

                    b.ToTable("AvaliacoesPIGE");
                });

            modelBuilder.Entity("COPI_API.Models.PIGEEntities.CicloPIGE", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<DateTime?>("DataFim")
                        .HasColumnType("datetime(6)");

                    b.Property<DateTime?>("DataInicio")
                        .HasColumnType("datetime(6)");

                    b.Property<bool?>("Encerrado")
                        .HasColumnType("tinyint(1)");

                    b.Property<string>("Nome")
                        .HasColumnType("longtext");

                    b.HasKey("Id");

                    b.ToTable("CiclosPIGE");
                });

            modelBuilder.Entity("COPI_API.Models.PIGEEntities.EixoPIGE", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<string>("Nome")
                        .HasColumnType("longtext");

                    b.Property<decimal>("Peso")
                        .HasColumnType("decimal(65,30)");

                    b.HasKey("Id");

                    b.ToTable("EixosPIGE");
                });

            modelBuilder.Entity("COPI_API.Models.PIGEEntities.KPIPIGE", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<int?>("EixoId")
                        .HasColumnType("int");

                    b.Property<int?>("NivelId")
                        .HasColumnType("int");

                    b.Property<string>("Nome")
                        .HasColumnType("longtext");

                    b.Property<double?>("Pontuacao")
                        .HasColumnType("double");

                    b.HasKey("Id");

                    b.HasIndex("EixoId");

                    b.HasIndex("NivelId");

                    b.ToTable("KPIsPIGE");
                });

            modelBuilder.Entity("COPI_API.Models.PIGEEntities.NivelPIGE", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<string>("Nome")
                        .HasColumnType("longtext");

                    b.Property<decimal>("Valor")
                        .HasColumnType("decimal(65,30)");

                    b.HasKey("Id");

                    b.ToTable("NiveisPIGE");
                });

            modelBuilder.Entity("COPI_API.Models.PIGEEntities.ResultadoKPIPIGE", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<string>("AvaliacaoEscrita")
                        .HasColumnType("longtext");

                    b.Property<int?>("AvaliacaoPIGEId")
                        .HasColumnType("int");

                    b.Property<int>("CicloPIGEId")
                        .HasColumnType("int");

                    b.Property<DateTime>("DataRegistro")
                        .HasColumnType("datetime(6)");

                    b.Property<int>("KPIPIGEId")
                        .HasColumnType("int");

                    b.Property<string>("Prova")
                        .HasColumnType("longtext");

                    b.Property<string>("Status")
                        .HasColumnType("longtext");

                    b.Property<DateTimeOffset?>("UltimaAlteracaoEm")
                        .HasColumnType("datetime(6)");

                    b.Property<string>("UltimaAlteracaoPor")
                        .HasColumnType("longtext");

                    b.Property<int>("UnidadeKPIPIGEId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("AvaliacaoPIGEId");

                    b.HasIndex("CicloPIGEId");

                    b.HasIndex("KPIPIGEId");

                    b.HasIndex("UnidadeKPIPIGEId");

                    b.ToTable("ResultadosKPIPIGE");
                });

            modelBuilder.Entity("COPI_API.Models.PIGEEntities.StatusPIGE", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<int>("CicloPIGEId")
                        .HasColumnType("int");

                    b.Property<int>("Status")
                        .HasColumnType("int");

                    b.Property<int>("UnidadeKPIPIGEId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("CicloPIGEId");

                    b.HasIndex("UnidadeKPIPIGEId");

                    b.ToTable("StatusPIGE");
                });

            modelBuilder.Entity("COPI_API.Models.PIGEEntities.UnidadeKPIPIGE", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<string>("SEI")
                        .HasColumnType("longtext");

                    b.Property<int>("UnidadeId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("UnidadeId");

                    b.ToTable("UnidadesKPIPIGE");
                });

            modelBuilder.Entity("ServidorUnidadeKPIPIGE", b =>
                {
                    b.Property<int>("ServidoresId")
                        .HasColumnType("int");

                    b.Property<int>("UnidadeKPIPIGEId")
                        .HasColumnType("int");

                    b.HasKey("ServidoresId", "UnidadeKPIPIGEId");

                    b.HasIndex("UnidadeKPIPIGEId");

                    b.ToTable("ServidorUnidadeKPIPIGE");
                });

            modelBuilder.Entity("COPI_API.Models.AdminEntities.Servidor", b =>
                {
                    b.HasOne("COPI_API.Models.AdminEntities.Divisao", "Divisao")
                        .WithMany("Servidores")
                        .HasForeignKey("DivisaoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Divisao");
                });

            modelBuilder.Entity("COPI_API.Models.DPEEntities.Documento", b =>
                {
                    b.HasOne("COPI_API.Models.DPEEntities.Afastamento", "Afastamento")
                        .WithMany()
                        .HasForeignKey("AfastamentoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("COPI_API.Models.AdminEntities.Divisao", "Divisao")
                        .WithMany()
                        .HasForeignKey("DivisaoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Afastamento");

                    b.Navigation("Divisao");
                });

            modelBuilder.Entity("COPI_API.Models.DPEEntities.Ementario", b =>
                {
                    b.HasOne("COPI_API.Models.DPEEntities.Afastamento", "Afastamento")
                        .WithOne("Ementario")
                        .HasForeignKey("COPI_API.Models.DPEEntities.Ementario", "AfastamentoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Afastamento");
                });

            modelBuilder.Entity("COPI_API.Models.MetaEntities.AcaoEstrategica", b =>
                {
                    b.HasOne("COPI_API.Models.MetaEntities.Meta", "Meta")
                        .WithMany("AcoesEstrategicas")
                        .HasForeignKey("MetaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Meta");
                });

            modelBuilder.Entity("COPI_API.Models.MetaEntities.Tarefa", b =>
                {
                    b.HasOne("COPI_API.Models.MetaEntities.AcaoEstrategica", "AcaoEstrategica")
                        .WithMany("Tarefas")
                        .HasForeignKey("AcaoEstrategicaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("AcaoEstrategica");
                });

            modelBuilder.Entity("COPI_API.Models.PIBPEntities.KPI", b =>
                {
                    b.HasOne("COPI_API.Models.PIBPEntities.Eixo", "Eixo")
                        .WithMany()
                        .HasForeignKey("EixoId");

                    b.HasOne("COPI_API.Models.PIBPEntities.Nivel", "Nivel")
                        .WithMany()
                        .HasForeignKey("NivelId");

                    b.Navigation("Eixo");

                    b.Navigation("Nivel");
                });

            modelBuilder.Entity("COPI_API.Models.PIBPEntities.ResultadoKPI", b =>
                {
                    b.HasOne("COPI_API.Models.PIBPEntities.Avaliacao", null)
                        .WithMany("Resultados")
                        .HasForeignKey("AvaliacaoId");

                    b.HasOne("COPI_API.Models.PIBPEntities.Ciclo", "Ciclo")
                        .WithMany()
                        .HasForeignKey("CicloId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("COPI_API.Models.PIBPEntities.KPI", "KPI")
                        .WithMany()
                        .HasForeignKey("KPIId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("COPI_API.Models.PIBPEntities.UnidadeKPI", "UnidadeKPI")
                        .WithMany("Resultados")
                        .HasForeignKey("UnidadeKPIId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Ciclo");

                    b.Navigation("KPI");

                    b.Navigation("UnidadeKPI");
                });

            modelBuilder.Entity("COPI_API.Models.PIBPEntities.StatusPIBP", b =>
                {
                    b.HasOne("COPI_API.Models.PIBPEntities.Ciclo", "Ciclo")
                        .WithMany()
                        .HasForeignKey("CicloId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("COPI_API.Models.PIBPEntities.UnidadeKPI", "UnidadeKPI")
                        .WithMany()
                        .HasForeignKey("UnidadeKPIId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Ciclo");

                    b.Navigation("UnidadeKPI");
                });

            modelBuilder.Entity("COPI_API.Models.PIBPEntities.UnidadeKPI", b =>
                {
                    b.HasOne("COPI_API.Models.AdminEntities.Servidor", "Servidor")
                        .WithMany("UnidadesKpiResponsaveis")
                        .HasForeignKey("ServidorId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("COPI_API.Models.AdminEntities.Unidade", "Unidade")
                        .WithMany()
                        .HasForeignKey("UnidadeId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Servidor");

                    b.Navigation("Unidade");
                });

            modelBuilder.Entity("COPI_API.Models.PIGEEntities.KPIPIGE", b =>
                {
                    b.HasOne("COPI_API.Models.PIGEEntities.EixoPIGE", "Eixo")
                        .WithMany()
                        .HasForeignKey("EixoId");

                    b.HasOne("COPI_API.Models.PIGEEntities.NivelPIGE", "Nivel")
                        .WithMany()
                        .HasForeignKey("NivelId");

                    b.Navigation("Eixo");

                    b.Navigation("Nivel");
                });

            modelBuilder.Entity("COPI_API.Models.PIGEEntities.ResultadoKPIPIGE", b =>
                {
                    b.HasOne("COPI_API.Models.PIGEEntities.AvaliacaoPIGE", null)
                        .WithMany("Resultados")
                        .HasForeignKey("AvaliacaoPIGEId");

                    b.HasOne("COPI_API.Models.PIGEEntities.CicloPIGE", "CicloPIGE")
                        .WithMany()
                        .HasForeignKey("CicloPIGEId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("COPI_API.Models.PIGEEntities.KPIPIGE", "KPIPIGE")
                        .WithMany()
                        .HasForeignKey("KPIPIGEId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("COPI_API.Models.PIGEEntities.UnidadeKPIPIGE", "UnidadeKPIPIGE")
                        .WithMany("Resultados")
                        .HasForeignKey("UnidadeKPIPIGEId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("CicloPIGE");

                    b.Navigation("KPIPIGE");

                    b.Navigation("UnidadeKPIPIGE");
                });

            modelBuilder.Entity("COPI_API.Models.PIGEEntities.StatusPIGE", b =>
                {
                    b.HasOne("COPI_API.Models.PIGEEntities.CicloPIGE", "CicloPIGE")
                        .WithMany()
                        .HasForeignKey("CicloPIGEId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("COPI_API.Models.PIGEEntities.UnidadeKPIPIGE", "UnidadeKPIPIGE")
                        .WithMany()
                        .HasForeignKey("UnidadeKPIPIGEId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("CicloPIGE");

                    b.Navigation("UnidadeKPIPIGE");
                });

            modelBuilder.Entity("COPI_API.Models.PIGEEntities.UnidadeKPIPIGE", b =>
                {
                    b.HasOne("COPI_API.Models.AdminEntities.Unidade", "Unidade")
                        .WithMany()
                        .HasForeignKey("UnidadeId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Unidade");
                });

            modelBuilder.Entity("ServidorUnidadeKPIPIGE", b =>
                {
                    b.HasOne("COPI_API.Models.AdminEntities.Servidor", null)
                        .WithMany()
                        .HasForeignKey("ServidoresId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("COPI_API.Models.PIGEEntities.UnidadeKPIPIGE", null)
                        .WithMany()
                        .HasForeignKey("UnidadeKPIPIGEId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("COPI_API.Models.AdminEntities.Divisao", b =>
                {
                    b.Navigation("Servidores");
                });

            modelBuilder.Entity("COPI_API.Models.AdminEntities.Servidor", b =>
                {
                    b.Navigation("UnidadesKpiResponsaveis");
                });

            modelBuilder.Entity("COPI_API.Models.DPEEntities.Afastamento", b =>
                {
                    b.Navigation("Ementario");
                });

            modelBuilder.Entity("COPI_API.Models.MetaEntities.AcaoEstrategica", b =>
                {
                    b.Navigation("Tarefas");
                });

            modelBuilder.Entity("COPI_API.Models.MetaEntities.Meta", b =>
                {
                    b.Navigation("AcoesEstrategicas");
                });

            modelBuilder.Entity("COPI_API.Models.PIBPEntities.Avaliacao", b =>
                {
                    b.Navigation("Resultados");
                });

            modelBuilder.Entity("COPI_API.Models.PIBPEntities.UnidadeKPI", b =>
                {
                    b.Navigation("Resultados");
                });

            modelBuilder.Entity("COPI_API.Models.PIGEEntities.AvaliacaoPIGE", b =>
                {
                    b.Navigation("Resultados");
                });

            modelBuilder.Entity("COPI_API.Models.PIGEEntities.UnidadeKPIPIGE", b =>
                {
                    b.Navigation("Resultados");
                });
#pragma warning restore 612, 618
        }
    }
}
